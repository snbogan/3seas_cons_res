...
T_psmc <- readRDS("psmcr_results_small_t.rds")
...
Full-scale psmc (not for students)

```{r}

b_full <- readRDS("data/vcf/psmcr_results.rds")

plot(b_full)

t_full <- readRDS("data/vcf/psmcr_resultst.rds")
...
Inbreeding coefficient using vcftools
...
het_df$Pop <- ifelse(grepl("^B_", het_df[[1]]), "BB",
                   ifelse(grepl("^T_", het_df[[1]]), "TB", NA))
...
ggplot(data = het_df,
  ...
  theme_classic() +
  facet_wrap(~Pop) +
  labs(y = "Individuals", x = "F (inbreeding coefficient)")


# What percent of samples had positive inbreeding coefficients?
nrow(filter(het_df, F > 0)) / nrow(het_df) * 100 # 38.46%

# How many individuals have F >0.1 where fitness costs my arise? 
nrow(filter(het_df, F > .1)) # 6 individuals

# Was there a difference in inbreeding between Tomales and Bodega Bay?
nrow(filter(het_df, F > 0 & Pop == "BB")) / nrow(filter(het_df, Pop == "BB")) * 100 # BB = 29.41%
nrow(filter(het_df, F > 0 & Pop == "TB")) / nrow(filter(het_df, Pop == "TB")) * 100 # BB = 43.86%

43.86/29.41

# Tomales Bay exhibited a 49% increase in positive inbreeding coefficients relative to Bodega Bay

```

Regress leaf sizes against inbreeding coeff and temperatures

```{r}

# Read in sample metadata
zm_meta <- read.csv("data/meta/Zm_TomBod_MAF01MM85INDM30_AllChr.metadata.csv")
...
het_meta$leaf_a <- as.numeric(het_meta$sheath_width)*
  as.numeric(het_meta$sheath_length)

# Plot F against leaf size
ggplot(data = het_meta,
       aes(y = as.numeric(sheath_width)*as.numeric(sheath_length), 
...
zm_brm <- brm(
  bf(leaf_a ~ scale(`F`) + scale(mean_temp_C) + Pop + depth +
...
posterior_interval(zm_brm, prob = 0.925)
